...
All outcomes are analysed using Bayesian statistical models. A detailed summary of the statistical models, including their prior specifications, is presented in the Appendix. Prior distributions are chosen to be weakly informative in the absence of information concerning the likely values of model parameters [Gelman]. 

The primary outcome peak TcCO~2~  is compared between the intervention and control groups using a robust regression model. Covariate adjustments for the stratification variables obstructive sleep apnea status and the presence of a cardiac resynchronization therapy device implant are made in this and in all other models made in all models. Baseline TcCO~2~ concentration is included as a covariate in this model, as is recommended for this type of design[@Egbewale2014], and it is modelled using splines. A subgroup analysis is performed by expanding the model to include treatment interactions with the covariates OSA and CRT. The results of the subgroup analysis are compared to those from the primary analysis with only main effects to investigate the robustness of the conclusions to model specification.

The secondary outcome mean TcCO~2~ concentration is a analysed using the same model used to analyze peak TcCO~2~. A functional analysis of variance (ANOVA) model [cite silverman, cite yue] is used to further investigate how mean TcCO~2~ concentration levels differ between the intervention and control groups as a function of procedure time.  The remaining continuous outcomes, average ISAS score and SPO~2~ AUC, are analyzed using the same model without a covariate adjustment for Baseline TcCO~2~. 

A logistic regression model is used for the binary outcome "patient experienced minor respiratory event". A proportional-odds model was used for ordinal outcomes patient comfort of oxygen delivery, and Anesthesia Assistant rating of difficulty maintaining oxygenation status and rating of difficulty using oxygen delivery device. The secondary outcome of mean TcCO~2~ was compared between the intervention and control groups using a Bayesian one-way functional analysis of variance (ANOVA) model [@yue2019].

Posterior inference for all models except the functional ANOVA model was performed using Hamiltonian Monte Carlo through the brms package [@bruckner2017], version `r packageVersion("brms")`. For this set of models, 2000 posterior samples were obtained from 4 independent chains of 2000 samples, where the first 1000 warm-up samples were discarded. Posterior inference for the functional ANOVA model was performed using the Integrated Nested Laplacian Approximation [@rue2009] through the INLA package, version `r packageVersion("INLA")`. The marginal posterior distribution of parameters are summarized by their mean and a 95% credible interval defined by the interval spanning the 2.5% and 97.5% percentiles of their distributions. The clinical significance of treatment effects relating to TcCO~2~ concentration are evaluated by computing the posterior probability that an effect exceeds 4 mmHg in either direction.

In the case of missing outcome data, we first assume the data to be missing completely at random (MCAR) [cite Rubin] and inference is performed using a complete-case analysis. When the proportion of missing data is large and the MCAR assumption is unlikely to be satisfied, a sensitivity analysis is performed to investigate the robustness of the conclusions of the complete-case analysis.
...
### Analysis
The results of the analysis and the numbers analyzed for all outcomes except the functional ANOVA model is presented in [Table]. The TcCO~2~ traces for all patients are displayed in Figure []. Three participants are missing TcCO~2~ concentration data, two because the TcCO~2~ sensor did not stabilize in time for the procedure start and one because their procedure was re-scheduled to a date when “no one was available to perform monitoring”.  Two participants are missing SPO~2~ levels, one because “” and the other being the previously mentioned participant whose procedure was rescheduled. The amount of missing data within TcCO~2~ and SPO~2~ trajectories is summarized in []. Due to the high amount of missing data observed in the SPO~2~ data the planned analysis of SPO~2~ AUC was not performed due to reliability concerns. 

The average ISAS scores from three participants are missing due to missing response for some of the component ISAS items. Complete data was observed on adverse sedation events and patient outcome with oxygen delivery.

The Anesthesia Assistant survey items “” and “” are missing 45 and 46 responses, respectively, likely due to the survey being voluntary. It is unlikely that missingness among Anesthesia Assistant ratings occurred completely at random, so a best- and worst-case imputation approach is used to investigate the impact that the missing data could have on the results of the complete-case analysis in extreme cases.

```{r}
readd(plot_fanova_data) %>% grid::grid.draw()
```
...
```{r}
readd(plot_fanova_effect)
```

### Primary outcome
The effect of treatment on the peak TcCO~2~ is estimated to be `r CI_co2_peak[["full"]]` and the probability that it exceeds the 4 mmHg clinical significance threshold in either direction is low. The results of the subgroup analysis for peak TcCO~2~  are presented in [Table]. The estimates for the treatment effect in the baseline, OSA, and CRT subgroups when treatment interactions are included in the model still do not exceed the clinical significance threshold in either direction with high probability. 

### Secondary outcomes
The effect of treatment on the mean TcCO~2~ concentration of the whole procedure is estimated to be `r CI_co2_mean[["full"]]` and the probability that it exceed the 4 mmHg clinical significance threshold is low. 

The effect of treatment on the mean TcCO~2~ concentration level over time is presented Figure []. It is observed that the estimated effect does not exceed the 4 mmHg clinical significance threshold  in either direction with high probability and there is no discernable trend observed in how the effect varies with procedure time. Note that the precision of the effect estimate decreases as procedure time increases, reflecting the shrinking number of mid-procedure participants to compare at those times.

The odds ratio of minor adverse airway or breathing outcome for the HFNO group compared with the FMO group is estimated by the logistic regression model to be `r CI_troops[["full"]]`. 

 The effect of treatment on average ISAS score was estimated by the robust regression model to be `CI_isas[["full"]]`, where the effect is measured in units of absolute difference.
 
 The odds ratio for Anesthesia Assistant ratings of difficulty maintaining oxygenation status and difficulty using oxygen delivery equipment as estimated using a complete-case analysis are `r CI_diffoxygen[["full"]]` and `r CI_diffuseoxygen[["full"]]`, where a value less than 1 indicates a greater level of difficulty for respondents in the HFNO group. The best- and worst-case sensitivity analysis gave estimates ranging between `r CI_diffoxygen_worst[["full"]]` and `r CI_diffoxygen_best[["full"]]` for difficulty maintaining oxygenation status and from `r CI_diffuseoxygen_worst[["full"]]` and `r CI_diffuseoxygen_best[["full"]]` for difficulty using the oxygen delivery device. These estimates suggest the directionality of the effect could be positive or negative with high probability, so conclusions of the complete-case analysis are not robust to assumptions about the values of the missing outcome.
